...
---
### 手机验证码登录
- 登陆页面输入手机号，点击“获取验证码”按钮，发送ajax请求，服务端调用短信服务API给指定手机发送验证码短信
- 登录页输入验证码，点击登录按钮，发送ajax请求，服务端处理登录请求
...
### 导入地址簿
即移动端消费者收货地址簿，只能有一个默认地址
- 要通过BaseContext.getCurrentId()拿到当前用户user_id
- 展示地址簿之前，要把这个user_id传给addressBook对象

### 菜品展示
- 根据左侧菜单的分类，展示菜品和套餐
- 页面发送ajax请求，获取分类category/list，同时获取购物车shoppingCart/list，
- 再发送ajax请求，获取菜品分类下的菜品，/dish/list
- 套餐下的菜品，/setmeal/list

### 购物车
将菜品/套餐添加到购物车，选择口味信息（菜品），+号添加，-号删除，
购物车中修改菜品/套餐的数量，可清空购物车
- 点击"加入购物车"按钮，或者+号按钮，发送ajax，菜品/套餐加购
- 点击 购物车 图标，发送ajax，查询购物车中的菜品套餐
- 点击“清空购物车”按钮，发送ajax，请求清空

### 下单
- 点击 去结算 按钮，跳转到订单确认页面，发送ajax，请求获取默认地址，以及购物车数据； 
- 点击 去支付按钮，完成下单操作（模拟，订单保存到数据库）